...
每两个月自动部署SSL证书到阿里云CDN上（主域名使用主域名证书，子域名使用泛域名证书）

## ✨ 功能特点

- 🔄 自动续期 Let's Encrypt 证书（为每个域名申请两个证书：主域名证书和泛域名证书）
- ☁️ 自动部署到阿里云 CDN
- ⏰ 每两个月自动运行一次
- 📧 证书过期邮件提醒
...
- **证书策略**：系统会为每个域名申请两个证书：
  ...
  - 部署时会自动判断：如果 `ALIYUN_CDN_DOMAINS` 中的域名是主域名，使用主域名证书；如果是子域名，使用泛域名证书

...
为这个用户创建一个 AccessKey (AK/SK)，并妥善保存
...
在左侧边栏中找到 "Secrets and variables" → "Actions"

点击 "New repository secret" 按钮，逐个添加以下秘密变量：
...
DOMAINS	主域名，多个用英文逗号隔开	example.com,test.org
ALIYUN_CDN_DOMAINS	CDN域名，与DOMAINS顺序对应，多个用英文逗号隔开	cdn.example.com,img.test.org
EMAIL	接收通知的邮箱地址	your-email@example.com
第四步：触发工作流运行
在您 Fork 的仓库页面，点击 "Code" 选项卡

找到任何一个文件（比如 README.md）并点击编辑按钮（铅笔图标）

随意做一些无意义的修改，比如在末尾加个空格或换行

填写提交信息（如 Trigger first run），然后点击 "Commit changes"

第五步：查看执行结果
提交完成后，点击仓库顶部的 "Actions" 选项卡

查看工作流运行状态和详细日志

如果所有配置正确，工作流会显示绿色的对勾（✅），表示执行成功

登录 阿里云CDN控制台，检查证书是否已更新
...
分隔符：多个域名之间使用英文逗号分隔，不要使用空格或其他符号

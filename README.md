...
每两个月自动部署SSL证书（支持主域名和泛域名）到阿里云CDN上

## ✨ 功能特点

- 🔄 自动续期 Let's Encrypt 证书（同时支持主域名和泛域名，如 example.com 和 *.example.com）
- ☁️ 自动部署到阿里云 CDN
- ⏰ 每两个月自动运行一次
- 📧 证书过期邮件提醒
...
| `ALIYUN_CDN_DOMAINS`       | CDN域名，与DOMAINS顺序对应，多个用**英文逗号**隔开。可以是主域名（如 example.com）或子域名（如 cdn.example.com） | `example.com,cdn.example.com` |
...
- **证书匹配**：证书同时包含主域名和泛域名（如 example.com 和 *.example.com），因此 `ALIYUN_CDN_DOMAINS` 中可以包含主域名或任何子域名，都能正常匹配
